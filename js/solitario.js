...
/*** !!!!!!!!!!!!!!!!!!! CODIGO !!!!!!!!!!!!!!!!!!!! **/
let botonReiniciar = document.getElementById("reset")
botonReiniciar.onclick = () => comenzarJuego()
    ...
    for (let i = 0; i < palos.length; i++ ) {
		for (let j = 0; j < numeros.length; j++) {
			const carta = {
				numero: numeros[j],
				palo: palos[i],
				src: `imagenes/baraja/${numeros[j]}-${palos[i]}.png`
			}
...
function barajar(mazo) {
	/*** !!!!!!!!!!!!!!!!!!! CODIGO !!!!!!!!!!!!!!!!!!!! **/
	return mazo.map(carta => ({carta,sort: Math.random()})).sort((a,b) => a.sort - b.sort).map(({carta}) => carta)
...
function cargarTapeteInicial(mazo) {
	...
	for (let j = 0; j < mazo.length; j++) {
		...
		imagen.src = mazo[j].src
		imagen.classList.add("no-ocupar-espacio")
		imagen.style.top = `${j * paso}px`
		imagen.style.left = `${j * paso}px`
		tapeteInicial.appendChild(imagen)
	}
